    ...
    class VenueProfilesController < BaseApiController
      ...
      def create
        if current_user.venue_profile.present?
          ...
          SpikerMailer.with(user: current_user, user_profile_name: current_user.venue_profile.name,
                            profile_type: 'Venue').user_registered_email.deliver_later
        end
      end
      ...
      def search
        ...
        render json: venues.sort_by { |venues| venues.name.downcase }
      ...
      def index
        venues = VenueProfile.where(hidden: false)
        render json: venues.order('lower(name) asc')
      end
